"""plex — Plex Media Server sessions, on-deck, recently-added, search.

Four GETs against the configured Plex server. Holds no logic beyond URL
templating and auth header injection.

Env (profile .env): PLEX_URL (e.g. http://10.0.1.10:32400), PLEX_TOKEN (X-Plex-Token).
"""
...
_PLEX_HDR = {"X-Plex-Token": "{env.PLEX_TOKEN}", "Accept": "application/json"}
...
def register(ctx) -> None:
    ...
    print(
        f"[plex] registered {len(TOOLS)} tools -> {_env('PLEX_URL') or '(PLEX_URL unset)'}",
        flush=True,
    )