...
Writes: `fin3_set_income_amount`, `fin3_set_account_balance`,
`fin3_set_investment_value`, `fin3_log_tracked_income`,
`fin3_update_tracked_income`, `fin3_log_expense`, `fin3_update_expense`,
`fin3_add_recurring`, `fin3_update_recurring`, `fin3_add_category`,
`fin3_update_category`, `fin3_set_category_icon`, `fin3_create_iou`,
`fin3_respond_iou`, `fin3_pay_iou`, `fin3_cancel_iou`.

**Income, accounts and investments are NARROWED to one number each** (2026-07-30).
alek: "I asked it to update the amount in my savings account. And what it did is
it updated the name of the savings account, but it left the balance as it was."
`fin3_add/update_{income,account,investment}` and `fin3_update_settings` were
REMOVED; creating and renaming those live in the web app behind a + button, and
the agent gets `fin3_set_income_amount`, `fin3_set_account_balance` and
`fin3_set_investment_value` — id and amount, nothing else. Currency and
frequency cannot be reached at all, so "change this to 325" has exactly one
meaning. Expenses (both kinds), categories and IOUs keep their full tools: those
are things the agent is genuinely better at than a form.
